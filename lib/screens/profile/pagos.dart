...
class _PagosScreenState extends State<PagosScreen> {
  ...
  Future<void> _fetchAbonos() async {
    ...
    try {
      final response = await TransactionRepository().getPagos(currentPage, 15);
  ...
  Widget _abonosList() {
    ...
    return ListView.builder(
      ...
      itemCount: pagos.length + (isLoading ? 1 : 0),
      itemBuilder: (context, index) {
        ...
        return _row(
          'TRANSACCIÓN #${pago['id']}',
          DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(pago['created_at'])),
          'Boleto ${pago['boleto']}',
          '\$${pago['monto']}',
          pago['estacionamiento']['nombre'],
        );
      },
    );
  }

  Widget _row(transaccion, fecha, lugar, monto, estacionamiento) {
    return Padding(
            ...
            SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(estacionamiento, style: AppTheme.theme.textTheme.bodyMedium!.copyWith(fontSize: 12, fontWeight: FontWeight.normal)),
                ],
              ),
            ),
